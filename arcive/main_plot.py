...
start = 0
start = input("press keybord to start \n")
...
def draw_data(i):
    ...
    print(str(string))
    string = string.strip(',')
    sensor_list = string.split(',', -1)
    sensor_list = list(map(float, sensor_list))
    # sensor_arr = np.array(sensor_list)
    # print(sensor_arr)
    x = np.full((sensor_arr.size,), sensor_arr[0])
    plt.cla()
    plt.scatter(x, sensor_arr)

ani = animation.FuncAnimation(plt.gcf(), draw_data , interval=10 )
time1 = time.time()
print(time1-time0)
# for i in range(50):
#     print(i)
